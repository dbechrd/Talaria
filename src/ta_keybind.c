...
{
    return keybind->key_state.down && !keybind->key_state.handled;
}
...
bool ta_keybind_pressed(const ta_keybind *keybind)
{
    ...
    return keybind->key_state.down && keybind->key_state.changed && !keybind->key_state.handled;
}
...
void ta_keybind_mark_handled(ta_keybind *keybind)
{
    if (keybind->keys[0]) ta_key_mark_handled(keybind->keys[0]);
    if (keybind->keys[1]) ta_key_mark_handled(keybind->keys[1]);
    if (keybind->keys[2]) ta_key_mark_handled(keybind->keys[2]);
    keybind->key_state.handled = true;
}
...
void ta_keybind_update(ta_keybind *keybind)
{
    ...
    if (keybind->key_state.changed) {
        keybind->key_state.handled = false;
