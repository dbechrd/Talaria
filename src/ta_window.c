...
void ta_window_init(ta_window *window, int w, int h, bool fullscreen)
{
    ...
    ta_log_write(&tg_debug_log, SRC_WINDOW, "gl3wInit...\n");
    int init = gl3wInit();
    if (init) {
        ta_log_write(&tg_debug_log, SRC_WINDOW, "gl3wInit failed with code %d", init);
    ...
    ta_log_write(&tg_debug_log, SRC_WINDOW, "Checking gl3w version support...\n");
    if (!gl3wIsSupported(gl_major, gl_minor)) {
        ta_log_write(&tg_debug_log, SRC_WINDOW, "OpenGL %d.%d not supported", gl_major, gl_minor);
    }
