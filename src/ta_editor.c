...
static editor_gizmo editor_gizmo_nearest()
{
            ...
            case WIDGET_TRANSLATE: {
                ...
                for (int i = 0; i < 3; ++i) {
                    if (ta_ray_v_aabb(&ray, &editor.gizmos.transform.hitbox1d[i], &t) && t < t_min) {
                        t_min = t;
                        nearest_gizmo = (editor_gizmo)((int)GIZMO_TRANSLATE_X + i);
                    }
                }
                ...
                for (int i = 0; i < 3; ++i) {
                    if (ta_ray_v_quad(&ray, &editor.gizmos.transform.hitbox2d[i], &t) && t < t_min) {
                        t_min = t;
                        nearest_gizmo = (editor_gizmo)((int)GIZMO_TRANSLATE_YZ + i);
                    }
                }
    ...
    if (!keep_changes) {
        ...
        if (selected_entity) {
            switch (editor.widget) {
                ...
                case WIDGET_ROTATE:
                case WIDGET_SCALE: {
                    ta_transform *e_transform = (ta_transform *)ta_game_component(selected_entity, RES_COMP_TRANSFORM);
...
static bool ta_editor_allow_entity_raycast()
{
    // Deny entity raycasts when:
    // - a gizmo is active
    // - a UI element is hovered
    bool deny = (
        editor.gizmo != GIZMO_NONE ||
        ta_ui_flag_hovered()
    );
...
const char *ta_editor_closest_entity()
{
    ...
    if (editor_gizmo_nearest(&ray)) {
        // Closest object is gizmo, not entity
        return 0;
    }
    ...
    ta_rigid_body *bodies = (ta_rigid_body *)ta_game_resource_pool(RES_COMP_RIGID_BODY);
...
#if _DEBUG && 0
    // DEBUG(cleanup): Draw pink sphere at intersection point on closest_entity surface
    ...
    // Don't allow selection of the currently active camera (to avoid self-intersections)
    if (closest_entity == tg_game.active_camera) {
        closest_entity = 0;
    }
...
static void editor_command_select()
{
    ...
    editor.gizmo = editor_gizmo_nearest();
    if (editor.gizmo) {
        ta_transform *e_transform = (ta_transform *)ta_game_component(editor.selected_entity, RES_COMP_TRANSFORM);
        editor.gizmo_start_xform = e_transform->xform;
    } else {
        const char *closest_entity = ta_editor_closest_entity();
...
{
    const char *selected_entity = editor.selected_entity;
    if (!selected_entity) {
        return;
    }
...
void ta_editor_draw_entity_wireframe(const char *entity, ta_rgba color, bool pulse_color)
{
    ta_camera *camera = ta_game_camera();
    ta_model *e_model = (ta_model *)ta_game_component_try(entity, RES_COMP_MODEL);
    if (e_model) {
        ta_shader *shader = (ta_shader *)ta_scene_find(&editor.scene, RES_SHADER, SYM(editor.shader_editor_select));
        ta_rgba wire_color = color;
        if (pulse_color) {
...
void ta_editor_draw_world()
{
    ...
    ta_editor_draw_entity_wireframe(selected_entity, TA_COLOR_YELLOW, true);
    ...
    switch (editor.widget) {
        case WIDGET_TRANSLATE: {
            ...
            static ta_rgba gizmo_color[GIZMO_COUNT][2] = {
            // TODO: Get rid of this if statement by moving this lookup table into a switch statement; probably need a
...
static void ui_node_panel()
{
    ...
    if (query_len) {
        ...
        switch (search_box.buffer[0]) {
            case '*':
                ...
                break;
            case '+': case '-':
                for (int i = 0; i < RES_COMP_COUNT; ++i) {
                    if (!strncmp(search_box.buffer + 1, res_type_lookup[i], query_len - 1) &&
                        query_len - 1 == strlen(res_type_lookup[i]))
                    {
                        if (search_box.buffer[0] == '+') {
                            res_filter_pos = (ta_res_type)i;
                            break;
                        } else if (search_box.buffer[0] == '-') {
                            res_filter_neg = (ta_res_type)i;
                            break;
                        }
                    }
                }
        ...
        ta_transform *exists = (ta_transform *)ta_game_by_name_try(RES_COMP_TRANSFORM, search_box.buffer, search_buf_len);
        if (exists) {
            ta_ui_next_bg_color(UI_STATE_NONE, 0.7f, 0.0f, 0.0f, 0.9f);
            ta_ui_next_bg_color(UI_STATE_INTERACT, 0.9f, 0.0f, 0.0f, 0.9f);
            ...
            if (ta_ui_button(CSTR("Create")) || search_box_submitted) {
                ...
                ta_transform *new_transform = (ta_transform *)ta_game_component_add(entity, RES_COMP_TRANSFORM,
                    search_box.buffer, search_buf_len);
    ...
    ta_transform *transform = (ta_transform *)ta_game_component(selected_entity, RES_COMP_TRANSFORM);
    ...
    if (model_expanded) {
        ta_model *model = (ta_model *)ta_game_component_try(selected_entity, RES_COMP_MODEL);
        if (model) {
            ta_ui_row_begin();
            ta_ui_next_size(label_width, 0);
            ...
            ta_ui_next_size(label_width, 0);
            ta_ui_label(CSTR("xform.position:"));
            static ta_ui_textbox_vec3_state model_xform_pos_editor = { 0 };
            ta_ui_textbox_vec3(&model->xform.position, &model_xform_pos_editor, false, true);

            ta_ui_row_begin();
            ta_ui_next_size(label_width, 0);
            ta_ui_label(CSTR("xform.orientation:"));
            // TODO: Allow editing rotations as Euler XYZ.
            static ta_ui_textbox_vec4_state model_xform_orient_editor = { 0 };
            ta_ui_textbox_vec4(&model->xform.orientation, &model_xform_orient_editor, true, true);

            ta_ui_row_begin();
            ta_ui_next_size(label_width, 0);
            ta_ui_label(CSTR("mesh:"));
            ta_ui_label(SYM(model->mesh));

            ta_ui_row_begin();
            ta_ui_label(CSTR("materials:"));
            dlb_vec_each(const char **, material, model->materials) {
                ta_ui_row_begin();
                ta_ui_next_margin_left(16);
                ta_ui_next_size(label_width, 0);
                ta_ui_label(SYM(*material));
            }

            if (model->morph_target_weights) {
                ta_ui_row_begin();
                ta_ui_next_size(label_width, 0);
                ta_ui_label(CSTR("morph_target_weights:"));
            ...
            ta_ui_toggle_button_end(&model->no_render);
            ...
            if (!model->no_render) {
                ...
                ta_ui_toggle_button_end(&model->no_shadow_cast);
            } else {
            ...
            if (ta_ui_button(CSTR("Add model"))) {
                ta_model *new_model = (ta_model *)ta_game_component_add(selected_entity, RES_COMP_MODEL,
                    SYM(selected_entity));
                ta_model_init(new_model);
            }
        }
    }
    ...
    if (rigid_body_expanded) {
        ta_rigid_body *rigid_body = (ta_rigid_body *)ta_game_component_try(selected_entity, RES_COMP_RIGID_BODY);
        if (rigid_body) {
            ta_ui_row_begin();
            ta_ui_next_size(label_width, 0);
            ...
            if (ta_ui_button(CSTR("Add rigid body"))) {
                ta_rigid_body *new_body = (ta_rigid_body *)ta_game_component_add(selected_entity, RES_COMP_RIGID_BODY,
                    SYM(selected_entity));
    ...
    if (light_expanded) {
        ta_light *light = (ta_light *)ta_game_component_try(selected_entity, RES_COMP_LIGHT);
        if (light) {
            ta_ui_row_begin();
            ta_ui_next_size(label_width, 0);
            ta_ui_label(CSTR("enabled:"));
            ta_ui_toggle_button(CSTR("On"), CSTR("Off"), &light->disabled);
            ...
            bool *no_shadow_cast = NULL;
            switch (light->type) {
                case TA_LIGHT_DIRECTIONAL: {
                    no_shadow_cast = &light->data.directional.no_shadow_cast;
                    break;
                } case TA_LIGHT_POINT: {
                    no_shadow_cast = &light->data.point.no_shadow_cast;
                    break;
                } case TA_LIGHT_SPOT: {
                    no_shadow_cast = &light->data.spot.no_shadow_cast;
                    break;
                }
            }

            if (no_shadow_cast != NULL) {
                ta_ui_row_begin();
                ta_ui_next_size(label_width, 0);
                ta_ui_label(CSTR("cast shadows:"));
                if (light->disabled) {
                    ta_ui_next_bg_color_rgba(UI_STATE_ALL, TA_COLOR_GRAY4);
                    ta_ui_button(CSTR("False"));
                    if (ta_ui_last_state().hover) {
                        ta_ui_tooltip(CSTR("Light must be enabled to cast shadows"));
                    }
                } else {
                    ta_ui_toggle_button(CSTR("True"), CSTR("False"), no_shadow_cast);
                }
            }
            ...
            if (light->disabled) {
                ta_ui_row_begin();
                ta_ui_next_bg_color_rgba(UI_STATE_ALL, TA_COLOR_GRAY4);
                ta_ui_button(CSTR("[+] Shadow map"));
                if (ta_ui_last_state().hover) {
                    ta_ui_tooltip(CSTR("Light must be enabled to see shadow map"));
                }
            } else if (no_shadow_cast != NULL && *no_shadow_cast) {
                ta_ui_row_begin();
                ta_ui_next_bg_color_rgba(UI_STATE_ALL, TA_COLOR_GRAY4);
                ta_ui_button(CSTR("[+] Shadow map"));
                if (ta_ui_last_state().hover) {
                    ta_ui_tooltip(CSTR("Light must cast shadows to see shadow map"));
                }
            } else {
                ta_ui_row_begin();
...
static void ui_audio_panel()
{
    ...
    ta_ui_row_begin();
    dlb_vec_each(ta_audio_buffer *, audio_buffer, (ta_audio_buffer *)ta_game_resource_pool(RES_AUDIO_BUFFER)) {
#if 0
        int node_panel_id = -1;
    ...
    if (audio_request_name) {
        ta_audio_source *bg_music_src = (ta_audio_source *)ta_game_component(tg_e_background_music,
...
static void ui_camera_panel()
{
    ...
    if (selected_camera) {
        ta_camera *camera = (ta_camera *)ta_game_by_sym(RES_COMP_CAMERA, selected_camera);
        ...
        ta_ui_label(CSTR("xform position"));
        ta_ui_label(CSTR("Target orientation"));
        ta_ui_label(CSTR("xform orientation"));
        ta_ui_label(CSTR("Target follow distance"));
        ta_ui_label(CSTR("Position smooth"));
        ta_ui_label(CSTR("Position target vel"));
        ta_ui_label(CSTR("Yaw"));
        ta_ui_label(CSTR("Yaw min"));
        ta_ui_label(CSTR("Yaw max"));
        ta_ui_label(CSTR("Yaw smooth"));
        ta_ui_label(CSTR("Yaw target"));
        ta_ui_label(CSTR("Pitch"));
        ta_ui_label(CSTR("Pitch min"));
        ta_ui_label(CSTR("Pitch max"));
        ...
        ta_ui_label_float(camera->pitch);
        ...
        for (size_t mode = 0; mode < ARRAY_SIZE(dbg_modes); ++mode) {
            ...
            if (ta_ui_toggle_button_end(&checked)) {
                camera->dbg_channel = (ta_glsl_dbg_channel)mode;
            }
        }
        ta_ui_panel_end();
        ta_ui_panel_end();
...
static void ui_material_details_panel(const char *material_name)
{
    ...
    ta_material *material = (ta_material *)ta_game_by_sym(RES_MATERIAL, material_name);
    ...
    ta_ui_label(CSTR("name"));
    ta_ui_label(SYM(material->name));

    ta_ui_row_begin();
    ta_ui_next_size(name_column_width, 0);
    ta_ui_label(CSTR("shader"));
    ta_ui_label(SYM(material->shader));

    ta_ui_row_begin();
    ta_ui_next_size(name_column_width, 0);
    ta_ui_label(CSTR("albedo_factor"));
    ...
    ta_ui_row_begin();
    ta_ui_next_size(name_column_width, 0);
    ta_ui_label(CSTR("albedo_texture"));
    ...
    ta_ui_row_begin();
    ta_ui_next_size(name_column_width, 0);
    ta_ui_label(CSTR("emission_factor"));
    ...
    ta_ui_row_begin();
    ta_ui_next_size(name_column_width, 0);
    ta_ui_label(CSTR("roughness_texture"));
    ta_ui_next_size(default_textbox_width, default_textbox_height);
    ...
    if (ta_ui_button(CSTR("Save"))) {
        #define FIND_OR_CREATE_TEXTURE_THEN_ASSIGN(textbox_buffer, material_property)                                       \
            ...
            if (name) {                                                                                                     \
                ...
                ta_texture *texture = (ta_texture *)ta_game_by_sym_try(RES_TEXTURE, name);                                  \
                if (!texture) {                                                                                             \
                    texture = (ta_texture *)ta_game_alloc(RES_TEXTURE, SYM(name));                                                        \
...
static void ui_material_panel()
{
    ...
    dlb_vec_each(ta_material *, material, (ta_material *)ta_game_resource_pool(RES_MATERIAL)) {
        ta_ui_row_begin();
...
static void ui_mesh_panel()
{
    ...
    dlb_vec_each(ta_mesh *, mesh, (ta_mesh *)ta_game_resource_pool(RES_MESH)) {
        ta_ui_row_begin();
...
static void ui_texture_panel()
{
    ...
    int count = 0;
    dlb_vec_each(ta_texture *, texture, (ta_texture *)ta_game_resource_pool(RES_TEXTURE)) {
        if (count % 5 == 0) {
            ta_ui_row_begin();
        }
        ...
        if (ta_ui_button_end()) {
            ...
            if (selected_entity) {
                ...
                if (model && model->material) {
                    ta_material *material = (ta_material *)ta_game_by_sym(RES_MATERIAL, model->material);
...
void ta_editor_draw_screen()
{
    ...
    if (collapsed) {
        ...
        ta_font *fonts = (ta_font *)ta_game_resource_pool(RES_FONT);
