...
bool ta_sphere_v_obb(ta_manifold *manifold, const ta_sphere *sphere, const ta_obb *obb)
{
    ...
    sphere_center_obb = quat_mul_vec3(quat_inverse(obb->orientation), sphere_center_obb);
    ...
    if (manifold) {
        ...
        ta_vec3 closest_world = vec3_add(closest, obb->center);
        ta_vec3 normal = vec3_sub(closest_world, sphere->center);
